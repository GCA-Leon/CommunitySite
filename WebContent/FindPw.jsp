...
<%	
	...
	try{
		...
		if(isEmail==true){//이메일이 존재하면
			...
			System.out.println("위에userEmail : "+ (String)request.getAttribute("userEmail"));
	...
	try{
		...
		if(isAttk==true){//인증되었으면
			System.out.println("인증키인증되었음");
...
%>
    <div id="wrap">
        ...
        <form id="findPW-form" action="UpdateInfo.do" method="post">
            <h1>이메일 주소로 비밀번호 찾기</h1>
            <h4>가입할 때 등록한 메일 주소를 입력하고 &nbsp;<span id="mailBtn">"메일 발송"</span>&nbsp; &nbsp;&nbsp; 버튼을 클릭해주세요.</h4>            
                <div class="txts">       
                   ...
                   <input type="button" value="메일 발송" id="sendMail" class="emailC"> 
                </div>
                <div class="txts">
                   <input type="text" id="vcode" value ="<%=session.getAttribute("userAttk")%>">
                   
                   <span data-placeholder="인증 번호"></span>                   
                   <input type="button" value="인증 하기" id="confire">
                ...
                <input type="button" class="signupbtn" value="변경 하기" id="update">
